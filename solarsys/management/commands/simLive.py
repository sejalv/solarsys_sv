...
class Command(BaseCommand):
    def handle(self, **options):
        ...
        #for n in range(today.hour+1): #range(24)
        today=today.replace(hour=today.hour,minute=00,second=00,microsecond=00) #hour=n or today.hour
        for item in ik:
            rdc_hr = ReferenceDC.objects.get(hour=today.hour)   #hour=n or today.hour
